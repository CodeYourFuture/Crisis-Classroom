...
const DeleteLesson = (req, res) => {
  ...
  deleteInstruction(lesson_id)
    .then(() => deleteTool(lesson_id))
    .then(() => deleteIngredient(lesson_id))
    .then(() => deleteLessons(lesson_id))
    .then(() => res.json({ msg: "Success! Your lesson has been deleted." }))
    .catch(err =>
      res.status(400).json({
        err,
  ...
  return new Promise((resolve, reject) => {
    pg.connect(
      ...
      (err, client, done) => {
        ...
        client
          .query(`DELETE FROM lessons where id=$1`, [lesson_id])
          .then(() => {
            return resolve();
          });
        done();
